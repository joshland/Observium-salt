...
#Change_php.config:
  /opt/observium/config.php:   
    file.blockreplace:
      ...
      - content: | 
          $config['db_user'] = '{{ pillar ['db_user']['user'] }}';
          $config['db_pass'] = '{{ pillar ['db_user']['password'] }}';
      - order: 17
 
  Create_db:
    cmd.run:
      - names:
        - mysql -u{{ pillar['mysql']['USERNAME'] }} -p{{ pillar['mysql']['PASS'] }} -e "CREATE DATABASE observium DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;"
  ...
  Change_privilages:
    cmd.run:
      - names:
        - mysql -u{{ pillar['mysql']['USERNAME'] }} -p{{ pillar['mysql']['PASS'] }} -e "GRANT ALL PRIVILEGES ON observium.* TO 'observium'@'localhost'IDENTIFIED BY '{{ pillar ['db_user']['password'] }}';"
#        -  mysql -u{{ pillar['mysql']['USERNAME'] }} -p{{ pillar['mysql']['PASS'] }} -e "exit;"
  ...
  /etc/apache2/sites-available/default:
    file.managed:
      - source: salt://observium/apache-default
      - order: 19
